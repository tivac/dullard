...
_.extend(Build.prototype, {
    ...
    addConfig : function(config) {
        ...
        if(config.steps) {
            ...
            this.steps = _.merge(this.steps, config.steps);
        }
        
        // Merge this config into existing config
        // Only dirs needs to be treated specially
        _.forOwn(config, function(value, key) {
            if(key === "dirs") {
                self._config.dirs =
                    ("dirs" in self._config) ?
                        self._config.dirs.concat(value) :
                        value;
            ...
            if(key in self._config) {
                self._config[key] = _.merge(self._config[key], value);
            } else {
